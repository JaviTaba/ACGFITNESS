...
export function buildWorkoutLogHandlers(
  ...
  const POST = async (request: Request) => {
    try {
      ...
      const result = await trackingService.logWorkout(body);
      ...
      return NextResponse.json(
        { error: "Unexpected error logging workout." },
        { status: 500 },
      );
    }
  };
...
export const { POST } = buildWorkoutLogHandlers();
