...
export function buildWeightLogHandlers(
  ...
  const POST = async (request: Request) => {
    try {
      ...
      const result = await trackingService.logWeight(body);
      ...
      return NextResponse.json(
        { error: "Unexpected error logging weight." },
        { status: 500 },
      );
    }
  };
...
export const { POST } = buildWeightLogHandlers();
