...
export function buildMealLogHandlers(
  ...
  const POST = async (request: Request) => {
    try {
      ...
      const result = await trackingService.logMeal(body);
      ...
      return NextResponse.json(
        { error: "Unexpected error logging meal." },
        { status: 500 },
      );
    }
  };
...
export const { POST } = buildMealLogHandlers();
